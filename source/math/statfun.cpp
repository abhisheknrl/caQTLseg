...
double Stat_GetMean_double(const CVector<double> &vData)
{
	// Returns the standard estimate of the mean.
	if (!vData.GetSize())
		return 0;
	return Stat_GetSum_double(vData, vData.GetSize()) / vData.GetSize();
}
...
bool Stat_NormalizeMatrix(CMatrix<double> &m, int mode, int cmin, int cmax)
{
	...
	else if (mode==NORM_Lognormal || mode==NORM_FPKMquantiles)
	{
		for (int i=0;i<n;i++)
	...
	for (int c=0;c<m.GetWidth();c++)
	{
		if (c>=cmin && c<=cmax)
		{
			...
			if (mode==NORM_FPKMquantiles)
			{
				double s= 0;
				if (true)
				{
					int n=0;
					for (double q=0.50;q<=0.95;q+=0.025)
					{
						int i= vSI.GetSize()*q;
						s += pSI[i].m_Value/icdf[i];
						printf("%g\t%g\t%g\t%g\n", q, pSI[i].m_Value, icdf[i], pSI[i].m_Value/icdf[i]);
						n++;
					}
					s /= n;
				}
				...
				{
					double q95= pSI[int(vSI.GetSize()*0.95)].m_Value;
					double q90= pSI[int(vSI.GetSize()*0.90)].m_Value;
					double q85= pSI[int(vSI.GetSize()*0.85)].m_Value;
					double q80= pSI[int(vSI.GetSize()*0.80)].m_Value;
					double q75= pSI[int(vSI.GetSize()*0.75)].m_Value;
					s= (q95/69.5+q90/36.6+q85/24.0+q80/17.5+q75/13.1)/5;
				}
				printf("scalefactor[%d]= %g\n", c, s);

				for (int i=0;i<n;i++)
					m.SetAt(pSI[i].m_nIndex, c, pSI[i].m_Value/s);
			}
			else
			{
				for (int i=0;i<n;i++)
