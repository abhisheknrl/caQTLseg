...
bool Table_ScanDouble(const char *ach, double &x)
{
	if (ach[0]=='-')
	{
		if (ach[1]=='1' && ach[2]=='.' && ach[3]=='#')
		{
			if (ach[4]=='I' && ach[5]=='N' && ach[6]=='F')
			{
				x= Stat_GetNegInfty_double();
				return true;
			}
			else if (ach[4]=='Q' && ach[5]=='N' && ach[6]=='A' && ach[7]=='N')
			{
				x= Stat_GetNaN_double();
				return true;
			}
			x= Stat_GetNaN_double();
			return false;
...
bool Table_ScanMatrix(const CTableIndex &ti,
						  int row0, int col0,
						  int row1, int col1,
						  CMatrix<double> &mValues, 
						  CMatrix<short> *pFlags /* can be NULL */)
{
	...
	// Parse floats.
	const double NaN= Stat_GetNaN_double();
	double *p, *p_end;
	...
	for (int r=row0;r<=row1;r++)
	{
		...
		for (int c=col0;c<=col1;c++)
		{
			...
			if (ach0!=ach1 && !(strncmp("NULL", ach0, 4)==0 || strncmp("NA", ach0, 2)==0))
			{
				double value;
				char tmp= *ach1;
				*ach1= 0; // Temporary null terminator

				if (ach1==Table_CheckFloatFormat(ach0) && Table_ScanDouble(ach0, value))
					*p++= value;
				else
				{
					printf("r=%d, c=%d, s='%s'\n", r, c, ach0);
...
bool Table_ScanTabDataset(const CTableIndex &idx, int &nAnnotRows, int &nAnnotCols, CMatrix<double> &mData)
{
	//
	...
	if (!Table_ScanMatrix(idx, nAnnotRows, nAnnotCols, idx.GetRowCount()-1, idx.GetMinColCount()-1, mData, &mFlags))
		return false;
...
CLoadBuf *Table_LoadSpaceDataset(CString sFilename, CTableIndex &ti, CString &sErr)
{
	CString sExt= g_FileSystem.GetFileExtension(sFilename);
	if (!g_FileSystem.FileExists(sFilename) && sExt.IsEmpty())
		sFilename += ".txt";
	...
	if (ti.ScanSpaceSeparated(pLB->m_aLoadBuf, NULL))
		return pLB;
	else
	{
		sErr= "Unable to parse dataset";
...
CString g_cmp_int;
Table_SortItem_int g_tmp_int;

void Quicksort_Ascending_String_int(Table_SortItem_int *pLeft, Table_SortItem_int *pRight)
{
	if (pRight<=pLeft)
		return;

	Table_SortItem_int *pi= pLeft-1;
	Table_SortItem_int *pj= pRight+1;

	g_cmp_int= pLeft->m_sKey;

	do 
	{
		while ((++pi)->m_sKey<g_cmp_int);
		while ((--pj)->m_sKey>g_cmp_int);
	...
	*pj= g_tmp_int;
	...
	Quicksort_Ascending_String_int(pLeft, pj);
	Quicksort_Ascending_String_int(pj+1, pRight);
}

void Quicksort_Descending_String_int(Table_SortItem_int *pLeft, Table_SortItem_int *pRight)
{
	if (pRight<=pLeft)
		return;
	...
	*pj= g_tmp_int;
	...
	Quicksort_Descending_String_int(pLeft, pj);
	Quicksort_Descending_String_int(pj+1, pRight);
}
...
void Quicksort_Ascending_Numerical(Table_SortItem *pLeft, Table_SortItem *pRight)
{
	...
	ASSERT(IsN_double(pLeft->m_fValue));
	ASSERT(IsN_double(pRight->m_fValue));
	do 
	{
		while ((++pi)->m_fValue<v);
		while ((--pj)->m_fValue>v);
...
void Quicksort_Ascending_Numerical_double(Table_SortItem_double *pLeft, Table_SortItem_double *pRight)
{
	...
	ASSERT(IsN_double(pLeft->m_Value));
	ASSERT(IsN_double(pRight->m_Value));
	do 
	{
		while ((++pi)->m_Value<v);
		while ((--pj)->m_Value>v);
...
void Quicksort_Ascending_Numerical_int(Table_SortItem_int *pLeft, Table_SortItem_int *pRight)
{
	if (pRight<=pLeft)
		return;
	...
	*pj= tmp;
	...
	Quicksort_Ascending_Numerical_int(pLeft, pj);
	Quicksort_Ascending_Numerical_int(pj+1, pRight);
}

void Quicksort_Descending_Numerical_int(Table_SortItem_int *pLeft, Table_SortItem_int *pRight)
{
	if (pRight<=pLeft)
		return;
	...
	*pj= tmp;
	...
	Quicksort_Descending_Numerical_int(pLeft, pj);
	Quicksort_Descending_Numerical_int(pj+1, pRight);
}
...
void RandomizeItems_int(Table_SortItem_int *pSI, int N)
{
	if (N<50)
		return; // Vector is small, don't bother about randomizing
...
void Table_SortItems_double(CVector<Table_SortItem_double> &vSI, bool bAscending, bool bNumerical)
{
	// Randomized quicksort (entry-point)
	Table_SortItem_double *pSI= vSI.GetBuffer();
	Table_SortItems_double(pSI, vSI.GetSize(), bAscending, bNumerical);
}

void Table_SortItems_int(Table_SortItem_int *pSI, int n, bool bAscending, bool bNumerical)
{
	// Randomized quicksort
	RandomizeItems_int(pSI, n);

	Table_SortItem_int *pLast= pSI + n-1;
	if (bNumerical)
	{
		if (bAscending)
			Quicksort_Ascending_Numerical_int(pSI, pLast);
		else
			Quicksort_Descending_Numerical_int(pSI, pLast);
	}
	else
	{
		if (bAscending)
			Quicksort_Ascending_String_int(pSI, pLast);
		else
			Quicksort_Descending_String_int(pSI, pLast);
	}
}

void Table_SortItems_int(CVector<Table_SortItem_int> &vSI, bool bAscending, bool bNumerical)
{
	// Randomized quicksort (entry-point)
	Table_SortItem_int *pSI= vSI.GetBuffer();
	Table_SortItems_int(pSI, vSI.GetSize(), bAscending, bNumerical);
}
								   ...
								   int nCol, bool bAscending, bool bNumerical)
{
	...
	for (int r=nAnnotRows;r<vSI.GetSize();r++)
	{
		...
		if (bNumerical)
			Table_ScanDouble(s, si.m_Value);
		else
		{
			si.m_sKey= s;
			si.m_Value= Stat_GetNaN_double();
		}
...
Table_SortItem_double *Table_InitSortItem(CVector<Table_SortItem_double> &vSI, 
								   CTableIndex &idx, int nAnnotRows, 
								   int nCol, bool bAscending, bool bNumerical)
{
	if (nCol<idx.GetMinColCount())
...
bool Table_GetDoubleAt(const CTableIndex &i, const int r, const int c, double &value)
{
	// Slow. To convert full matrices, use Table_ScanFloatMatrix instead.
	CString s= Table_GetAt(i, r, c);
	if (Table_CheckFloatFormat(s)!=NULL && Table_ScanDouble(s, value))
		return true;
	return false;
}
...
int Table_FindAnnotCol(const CTableIndex &ti, int nRow, const CString &sName, int nAnnotCols)
{
	...
	{
		CString si= Table_GetAt(ti,nRow,i);
		//for (int c=0;c<si.GetLength();c++)
		//	printf("%d\n", int(si[c]));
		// printf("si='%s', s= '%s'\n", si, s);
		if (si==s)
			return i;
...
bool Table_CenterByClass(const CTableIndex &ti, const int nAnnotCols, CMatrix<double> &mData, const int nClassRow)
{
	CVector<CString> vClassNames;
	CVector<int> vClassCount;
	CVector<int> vColumnClass;
	...
	{
		printf("Internal error: Table_EnumerateClasses failed\n");
		return false;
	}
	if (vColumnClass.GetSize()!=mData.GetWidth())
	{
		printf("Internal error: vColumnClass.GetSize()!=mData.GetWidth()\n");
		return false;
	}

	for (int i=0;i<vClassNames.GetSize();i++)
		printf("stratification class[%d]= '%s'\n", i, (const char *)vClassNames[i]);
	...
	CVector<double> v_median(vColumnClass.GetSize());
	double *p_median= v_median.GetBuffer();

	for (int r=0;r<mData.GetHeight();r++)
	{
		double *pr= mData.GetPointer(r, 0);
		...
		else
		{
			...
			for (int c=0;c<vClassNames.GetSize();c++)
			{
				...
				pn[c]= n;
				ps[c]= Stat_GetMedian_double(p_median, pn[c]);
				// printf("pn[%d]= %d, median[%d]= %g\n", c, pn[c], c, ps[c]);
			}
		}

		// center
		for (int i=0;i<vColumnClass.GetSize();i++)
			pr[i] -= vs[vColumnClass[i]];
	}

	return true;
}

int Table_FindInIndex_double(const char *ach, const CVector<Table_SortItem_double> &vSI /* sorted by m_sKey in ascending order */)
{
