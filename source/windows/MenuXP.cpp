/*******************************************************************************
 *
 *   MenuXP.cpp -- Office XP-style menu class.
 *
 *   Björn Nilsson, 2003
 *
 *	 Based on example from codeproject.com
 */
...
// IMGWIDTH, IMGHEIGHT = (16,16) by default. 
// However, skw uses larger icons.
#define IMGWIDTH 20
#define IMGHEIGHT 18
#define IMGPADDING 6
...
// Redefine to 4 in order to get a shadow under non-XP systems.
// Setting it to non-zero yields a strange double shadow 
// under XP (the second one is rendered by the themes manager).
#define SM_CXSHADOW 0

const TCHAR _WndPropName_OldProc[] = _T("XPWndProp_OldProc");
const TCHAR _WndPropName_MenuXP[] = _T("XPWndProp_MenuXP");

// Helper function imported from menuxp\tools.h
...
public:
    CMenuItemXP();
    CMenuItemXP(HMENU hMenu, UINT uItem, bool fByPosition = true);
   ~CMenuItemXP();
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
BYTE CMenuItemXP::ms_nCheck = 0;
CRect CMenuItemXP::ms_rcMRUMenuBarItem (0, 0, 0, 0);

///////////////////////////////////////////////////////////////////////////////
CMenuItemXP::CMenuItemXP()
{
    memset (&m_miInfo, 0, sizeof(MENUITEMINFO));
}

///////////////////////////////////////////////////////////////////////////////
CMenuItemXP::CMenuItemXP(HMENU hMenu, UINT uItem, bool fByPosition)
{
    memset (&m_miInfo, 0, sizeof(MENUITEMINFO));
...
///////////////////////////////////////////////////////////////////////////////
int CMenuItemXP::GetCaption (CString& sCaption) const
{
...
///////////////////////////////////////////////////////////////////////////////
int CMenuItemXP::GetShortCut (CString& sShortCut) const
{
    ASSERT(m_miInfo.fMask & MIIM_TYPE);
...
///////////////////////////////////////////////////////////////////////////////
bool CMenuItemXP::GetSeparator () const
{
    ASSERT(m_miInfo.fMask & MIIM_TYPE);
...
///////////////////////////////////////////////////////////////////////////////
bool CMenuItemXP::GetChecked () const
{
...
///////////////////////////////////////////////////////////////////////////////
bool CMenuItemXP::GetDisabled () const
{
    ASSERT(m_miInfo.fMask & MIIM_STATE);
...
///////////////////////////////////////////////////////////////////////////////
bool CMenuItemXP::GetDefault () const
{
    ASSERT(m_miInfo.fMask & MIIM_STATE);
...
///////////////////////////////////////////////////////////////////////////////
HMENU CMenuItemXP::GetPopup () const
{
...
///////////////////////////////////////////////////////////////////////////////
int CMenuItemXP::GetCaptionWidth (CDC* pDC) const
{
    if ( GetSeparator() )
    {
        return 0;
...
///////////////////////////////////////////////////////////////////////////////
int CMenuItemXP::GetShortCutWidth (CDC* pDC) const
{
    if ( GetSeparator() )
    {
        return 0;
...
///////////////////////////////////////////////////////////////////////////////
int CMenuItemXP::GetHeight (CDC* pDC) const
{
    TEXTMETRIC tm;
...
///////////////////////////////////////////////////////////////////////////////
bool CMenuItemXP::Draw (CDC* pDC, LPCRECT pRect, bool bSelected, bool bMenuBar, bool bHotLight, bool bInactive, bool bNoAccel) const
{
    ...
    if ( bMenuBar && bSelected )
    {
        ...
        //CMenuItemXP::ms_hMnuHeader = GetPopup();
        pDC->Rectangle (rc);
        rc.left = rc.right;
        rc.right += TEXTPADDING_MNUBR;
            ...
            else
            {
                ...
                if ( m_ImgDesc.m_hImgList != NULL && m_ImgDesc.m_nIndex != -1 )
                {
                    if ( GetDisabled() || (bSelected && !GetChecked()) )
                    {
                        ...
                        pDC->DrawState (CPoint (pRect->left + ( bOver ? 2 : 1 ), rc.top + ( bOver ? 2 : 1 )),
                ...
                else if ( GetChecked() )
                {
                    ...
                    rc.right = rc.left + IMGWIDTH-1;//+IMGPADDING;
					...
					if (hBmp)
					{
						...
						CPoint szPos( (rc.left + rc.right - b.bmWidth)/2-2, (rc.top + rc.bottom - b.bmHeight)/2 ) ;
						pDC->DrawState (szPos, CSize(rc.Size()), hBmp, DSS_NORMAL, (HBRUSH)NULL);
					}
                    DeleteObject (hBmp);
                }
            }
...
//    TRACE(_T("Referenced captions : %i\n"), ms_sCaptions.GetCount()+ms_sSubMenuCaptions.GetCount());
    ...
    if ( pWnd != NULL && !bMenuBar )
    {
        if ( pWnd->IsKindOf (RUNTIME_CLASS(CMDIFrameWnd)) )
        {
            ...
            {
                CMenuItemXP::ms_rcMRUMenuBarItem.SetRectEmpty();
                return;
            }
        }
    ...
    for ( int i = 0; i < nItemCount; i++ )
    {
        ...
        if ( (mii.fType & MFT_OWNERDRAW) == 0 && (!bMenuBar || (mii.fType & MFT_BITMAP) == 0) )
        {
            ...
            if ( bMenuBar )
            {
                ...
                cDC.DrawText (sCaption, _tcslen (sCaption), rcText, DT_SINGLELINE|DT_LEFT|DT_CALCRECT);
...
void CMenuXP::OnDrawItem (DRAWITEMSTRUCT* pDrawItemStruct, HWND hWnd)
{
    ASSERT (pDrawItemStruct->CtlType == ODT_MENU);
    CBufferDC cDC (pDrawItemStruct->hDC, pDrawItemStruct->rcItem);
    CMenuItemXP item ((HMENU)pDrawItemStruct->hwndItem, pDrawItemStruct->itemID, false);
    CFontDC font (cDC, DEFAULT_GUI_FONT);
    ...
    if ( item.Draw (&cDC, &pDrawItemStruct->rcItem, (pDrawItemStruct->itemState&ODS_SELECTED)!=0, LOBYTE(LOWORD(pDrawItemStruct->itemData))==0, (pDrawItemStruct->itemState&ODS_HOTLIGHT)!=0, (pDrawItemStruct->itemState&ODS_INACTIVE)!=0, (pDrawItemStruct->itemState&ODS_NOACCEL)!=0) )
    {
        ...
        ::ClientToScreen (hWnd, CMenuItemXP::ms_rcMRUMenuBarItem);
        CMenuItemXP::ms_rcMRUMenuBarItem.top = rc.top;
        CMenuItemXP::ms_rcMRUMenuBarItem.bottom = rc.bottom;
    }
    CMenuItemXP::ms_nCheck++;
}
...
/******************************************************************************/
//	CWndMenuXP class : 
//	Management of the window used by system to display popup menus
//
...
void CWndMenuXP::OnPrint (CDC* pDC, bool bOwnerDrawnItems)
{
    ...
    if ( bOwnerDrawnItems && !CMenuItemXP::ms_rcMRUMenuBarItem.IsRectEmpty() &&
         CMenuItemXP::ms_rcMRUMenuBarItem.bottom == m_ptMenu.y+1 )
    {
        pen.Color (HLS_TRANSFORM (::GetSysColor (COLOR_3DFACE), +20, 0));
        pDC->MoveTo (CMenuItemXP::ms_rcMRUMenuBarItem.left-m_ptMenu.x-3, 0);
        pDC->LineTo (CMenuItemXP::ms_rcMRUMenuBarItem.left-m_ptMenu.x+CMenuItemXP::ms_rcMRUMenuBarItem.Width()-5, 0);
    }
...
void CWndMenuXP::OnNcPaint ()
{
    ...
    if ( rc != m_rcMenu )
    {
		...
		// TRACE("Color2= %d\n", col);
        pen.Color (HLS_TRANSFORM (col, +75, 0));
        rc.DeflateRect (1, 1);
        pDC->Rectangle (rc);
        rc.DeflateRect (1, 1);
        pDC->Rectangle (rc);

        if ( !CMenuItemXP::ms_rcMRUMenuBarItem.IsRectEmpty() &&
             CMenuItemXP::ms_rcMRUMenuBarItem.bottom == m_ptMenu.y+1 )
        {
            pen.Color (HLS_TRANSFORM (::GetSysColor (COLOR_3DFACE), +20, 0));
            pDC->MoveTo (CMenuItemXP::ms_rcMRUMenuBarItem.left-m_ptMenu.x-3, 0);
...
LRESULT CALLBACK CWndMenuXP::SubClassMenuProc (HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PRINT:
        {
            BYTE nCheck = CMenuItemXP::ms_nCheck;
            LRESULT lResult = CallWindowProc (oldWndProc, hWnd, uMsg, wParam, lParam);
            bool bOwnerDrawnItems = nCheck != CMenuItemXP::ms_nCheck;
